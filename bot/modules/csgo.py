...
from utils import reaction_menu, net
...
class CSGO():
    ...
    @commands.command()
    async def csgo_update(self, ctx):
        response, _, code = await net.get_url("http://blog.counter-strike.net/index.php/category/updates/feed/")
        xml = await response.read()
        root = etree.fromstring(xml)
        for i, element in enumerate(root.xpath("//item")):
            if i == 0:
                print(dateparser.parse(element[2].text))
                a = dateparser.parse(element[2].text)
            if i == 3:
                print(dateparser.parse(element[2].text))
                b = dateparser.parse(element[2].text)
        print(a > b)
